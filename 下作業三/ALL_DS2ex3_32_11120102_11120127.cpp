...
string CharToString( char sid [] ) {
	string s = "" ;
	for ( int i = 0 ; sid[i] > 47 && sid[i] < 58 ; i += 1 ) 
		s += sid[i] ;
...
void quadraticProbingInsert(vector<HashEntry> &hashTable, Student& student){
    int originalIndex = hashFunc(string(student.sid), hashTable.size());
    ...
    if (!hashTable[index].occupied) {
        hashTable[index].sid = string(student.sid);
...
void SearchKey(vector<HashEntry> hashTable, string id){
	int originalIndex = hashFunc(id, hashTable.size());
	int index = originalIndex;
    int h = 1;
    bool hasfound = false;
    
    while (hashTable[index].occupied && id != hashTable[index].sid && h < hashTable.size()) {
        index = (originalIndex + h * h) % hashTable.size();
        h++;
    }
    
    if (id == hashTable[index].sid) hasfound = true;
    
    if ( hasfound ) cout << "{ " << hashTable[index].sid << ", " << hashTable[index].sname << ", " << hashTable[index].average << " } is found after " << h << " probes." << endl;
	else cout << id << " is not found after " << h << " probes." << endl;  
	
	cout << endl;
}

float averageSearchTimes(vector<HashEntry> &hashTable, const vector<Student> &studentData, int totalEntries, bool existing) {
  float loadFactor = 0;
  int count = 0;
...
int main(){
  ...
  int command = 0;
  int i = 0;
  string fileNum, student_id, dsid ;
  string id;
  vector<Student> Data;
      ...
      if ( command == 0 ) break;
        ...
        cout << endl << "Input a file number ([0] Quit): " ;
      	...
      	if ( fileNum == "0" ) cout << endl;
            ...
            cout.unsetf(ios_base::fixed); // 重設精準度 
	      ...
	      hash_table_size = FindPrimeNumber( Data ) ; // 用來找hash table適合的大小
      	  ...
      	  for ( int i = 0 ; i < Data.size() ; i += 1 ) {
      		...
      		if ( ! double_hash[remainder].occupied ) { // 該格沒被占用 
      			dsid = CharToString( Data[i].sid ) ;
      			double_hash[remainder].sid = dsid ; // 填資料 
...
//      			cout << "沒撞到 " << double_hash[remainder].time << double_hash[remainder].sname << endl ;
				...
				dsid = CharToString( Data[i].sid ) ;
				double_hash[remainder_and_step].sid = dsid ; // 填資料 
...
//      			cout << "  撞到 " << compare << " " << double_hash[remainder_and_step].sname << endl ;
	    ...
	    while ( true ) {
			...
			int probe = 1 ;
	    	remainder = hashFunc( student_id, hash_table_size ) ;
	    	if ( ! double_hash[remainder].occupied ) {
	   	 		cout << student_id << " is not found after " << probe << " probes." << endl ;
			}
		
			else {
				int step = highest_step - hashFunc( student_id, highest_step ) ;
				
				while ( true ) {
					if ( ! double_hash[remainder].occupied ) { // 空的 
						cout << student_id << " is not found after " << probe << " probes." << endl ;
						break ;
					}
					
					else if ( student_id == double_hash[remainder].sid ) { // 學號相同 
						cout << "{ " << double_hash[remainder].sid << ", " << double_hash[remainder].sname << ", " << double_hash[remainder].average ;
						cout << " } is found after " << probe << " probes." << endl ;
						break ;
					}
					
					probe += 1 ;
					remainder = ( remainder + step ) % hash_table_size ;
				}
			}
