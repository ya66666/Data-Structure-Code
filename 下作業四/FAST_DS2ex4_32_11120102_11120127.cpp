...
//			cout << "pop = " << id << endl ;
  ...
  void Writefile(string file, vector<Num> student_id) {
    ...
    oss << "<<< There are " << Data.size() << " IDs in total. >>>\n";
    
    for (size_t i = 0; i < Data.size(); ++i) {
        oss << "[" << setw(3) << setfill(' ') << i + 1 << "] " << student_id[i].str << "(" << student_id[i].cns << "): " << endl;
        
        for (size_t j = 0; j < Data[i].size(); ++j) {
            if (j % 12 == 0) oss << "\t"; 
            oss << "(" << setw(2) << setfill(' ') << j + 1 << ") " << Data[i][j] << "\t"; 
            if ((j + 1) % 12 == 0 && (j + 1) < Data[i].size()) oss << endl; 
        }
        oss << endl; 
    }

    ofstream fout(("pairs" + file + ".cnt").c_str(), ios::out | ios::trunc);
    fout << oss.str(); 
    fout.close();      
  } 
    ...
    do{
        ...
        if ( command == 0 ) break;
        	...
        	if (!again) {
				...
				All.Sort( student_id ) ;
				All.Writefile( fileNum, student_id ) ;
	  			again = true ;
	  		}
        	...
        	cin >> number;
			cout << endl << "<<< There are " << " IDs in total. >>>" << endl << endl;
        }
        else if ( command == 4 ) {
